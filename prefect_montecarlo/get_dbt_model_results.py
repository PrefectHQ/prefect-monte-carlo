...
Montecarlo query_get_dbt_model_results* tasks
"""
...
config_path = (
    Path(__file__).parent.resolve() / "configs" / "query" / "get_dbt_model_results.json"
)
return_fields_defaults = initialize_return_fields_defaults(config_path)


@task
async def query_get_dbt_model_results(  # noqa
    ...
    Args:
        run_start_time: Beginning of time window to filter run start times.
        run_end_time: End of time window to filter run start times.
        ...
        before: When paging backward: the cursor of the first item on the next
            page of results.
        status: Status(es) to filter run results.
    ...
    op_selection = op.get_dbt_model_results(
        **strip_kwargs(
    ...
    op_stack = ("getDbtModelResults",)
    op_selection = _subset_return_fields(
        op_selection, op_stack, return_fields, return_fields_defaults
    )

    result = await _execute_graphql_op(op, montecarlo_credentials)
    return result["getDbtModelResults"]


@task
async def query_get_dbt_model_results_edges(  # noqa
    run_start_time: datetime,
    run_end_time: datetime,
    montecarlo_credentials: MontecarloCredentials,
    first: int = None,
    ...
    Args:
        run_start_time: Beginning of time window to filter
            run start times.
        run_end_time: End of time window to filter run
            start times.
        montecarlo_credentials: Credentials to use for authentication with
            Montecarlo.
    ...
    result = await _execute_graphql_op(op, montecarlo_credentials)
    return result["getDbtModelResults"]["edges"]


@task
async def query_get_dbt_model_results_page_info(  # noqa
    run_start_time: datetime,
    run_end_time: datetime,
    montecarlo_credentials: MontecarloCredentials,
    first: int = None,
    ...
    result = await _execute_graphql_op(op, montecarlo_credentials)
    return result["getDbtModelResults"]["pageInfo"]
